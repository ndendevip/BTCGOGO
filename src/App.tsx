...
import { UnluckyRanking } from './components/UnluckyRanking';
...
function App() {
  ...
  const [winEffectType, setWinEffectType] = useState<'normal' | 'delay' | 'blackout'>('normal');
  ...
  const handleClick = () => {
    ...
    if (isWin) {
      ...
      if (effectRandom < 0.20) {
        setWinEffectType('delay');
        setTimeout(() => {
          setShowWinMessage(true);
        }, 1800);
      } else if (effectRandom < 0.65) {
